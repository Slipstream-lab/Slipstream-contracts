...
// ---------------------------------------------------------------------------
// Measured deltas with provenance (`harness bench`)
// ---------------------------------------------------------------------------

/// Marker opening the auto-generated measured block inside a `BENCH.md`.
pub const BENCH_BEGIN: &str = "<!-- BENCH-MEASURED-BEGIN -->";
/// Marker closing the auto-generated measured block inside a `BENCH.md`.
pub const BENCH_END: &str = "<!-- BENCH-MEASURED-END -->";

/// Environment and repository snapshot recorded alongside measured numbers.
///
/// Every field comes from the machine the command ran on or from `git`; values
/// that could not be determined are recorded as `unknown` rather than guessed,
/// so a `BENCH.md` never carries fabricated provenance.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BenchProvenance {
    pub os: String,
    pub arch: String,
    pub rustc: String,
    pub slipstream_version: String,
    /// `git rev-parse HEAD` of the contracts repo.
    pub contracts_sha: String,
    /// `git rev-parse HEAD` of slipstream-core (from `SLIPSTREAM_CORE_DIR`).
    pub core_sha: String,
    /// When the run happened (unix epoch seconds).
    pub run_time: String,
    /// The full command that produced the numbers.
    pub command: String,
}

/// Render a signed delta the way the bench tables display it (`+8`, `-1`, `0`).
...
/// Render the auto-generated measured block for one pattern.
///
/// The block is delimited by [`BENCH_BEGIN`]/[`BENCH_END`] markers so
/// [`update_bench_md`] can replace it idempotently. It does not end with a
/// trailing newline; the caller adds the file-level newline.
pub fn render_bench_block(
    pattern: &str,
    delta: &ContentionDelta,
    prov: &BenchProvenance,
) -> String {
    format!(
        "{BENCH_BEGIN}\n\n\
         ## Measured deltas\n\n\
         Raw `slipstream diff naive optimized --json` deltas for `{pattern}`, \
         generated by `harness bench`. Do not hand-edit; re-run the command to refresh.\n\n\
         | Metric | Delta |\n\
         | --- | --- |\n\
         | storage writes | {writes} |\n\
         | storage reads | {reads} |\n\
         | detector findings | {findings} |\n\n\
         _Provenance: OS={os}, arch={arch}, rustc={rustc}, slipstream={slip}, \
         contracts@{contracts_sha}, core@{core_sha}, run={run_time}, command=`{command}`_\n\n\
         {BENCH_END}",
        writes = signed_delta(delta.storage_writes_delta),
        reads = signed_delta(delta.storage_reads_delta),
        findings = signed_delta(delta.detector_findings_delta),
        os = prov.os,
        arch = prov.arch,
        rustc = prov.rustc,
        slip = prov.slipstream_version,
        contracts_sha = prov.contracts_sha,
        core_sha = prov.core_sha,
        run_time = prov.run_time,
        command = prov.command,
    )
}

/// Insert or replace the measured block in a `BENCH.md`, idempotently.
///
/// If the file already carries a block between [`BENCH_BEGIN`] and
/// [`BENCH_END`], that span is replaced. Otherwise the block is appended. The
/// rest of the file is never touched. Returns `true` when the file changed.
pub fn update_bench_md(path: &Path, block: &str) -> std::io::Result<bool> {
    ...
    let new =
        if let (Some(begin), Some(end)) = (existing.find(BENCH_BEGIN), existing.find(BENCH_END)) {
            let end = end + BENCH_END.len();
            format!("{}{}{}", &existing[..begin], block, &existing[end..])
        } else {
            let mut s = String::with_capacity(existing.len() + block.len() + 2);
            s.push_str(&existing);
